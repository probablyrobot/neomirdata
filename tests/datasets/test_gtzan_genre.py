...
def test_load_beats():
    beats_path = "tests/resources/mir_datasets/gtzan_genre/" + "gtzan_tempo_beat-main/beats/gtzan_country_00000.beats"
    ...
    assert np.array_equal(beat_data.positions, np.array([1, 2, 3, 4, 1])), "beat_data.positions different from expected"

    assert gtzan_genre.load_beats(None) is None, "load_beats(None) should return None"

    # check empty positions
    beats_path = (
        "tests/resources/mir_datasets/gtzan_genre/" + "gtzan_tempo_beat-main/beats/gtzan_country_00000_noposition.beats"
    )
    beat_data = gtzan_genre.load_beats(beats_path)
...
def test_load_tempo():
    tempo_path = "tests/resources/mir_datasets/gtzan_genre/" + "gtzan_tempo_beat-main/tempo/gtzan_country_00000.bpm"
