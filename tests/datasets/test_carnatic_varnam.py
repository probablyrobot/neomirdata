...
def test_track():
    ...
    expected_attributes = {
        ...
        "audio_path": os.path.join(
            os.path.normpath("tests/resources/mir_datasets/compmusic_carnatic_varnam/carnatic_varnam_1.1/"),
            os.path.normpath("Audio/223578__gopalkoduri__carnatic-varnam-by-dharini-in-abhogi-raaga.mp3"),
        ),
...
def test_load_notation():
    ...
    with pytest.raises(FileNotFoundError):
        parsed_data = compmusic_carnatic_varnam.load_notation("a/fake/path", taala_path, structure_path)
    with pytest.raises(FileNotFoundError):
        parsed_data = compmusic_carnatic_varnam.load_notation(notation_path, "a/fake/path", structure_path)
    with pytest.raises(FileNotFoundError):
        parsed_data = compmusic_carnatic_varnam.load_notation(notation_path, taala_path, "a/fake/path")
